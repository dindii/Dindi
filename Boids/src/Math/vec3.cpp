...
namespace Boids
{
	...
	vec3 vec3::Normalize(const vec3& vec)
	{
		...
		{
			GLR_LOG_ERROR("Cannot divide by zero");
			return vec;
		}
	}
	...
	Boids::vec3 vec3::lerp(const vec3& start, const vec3& end, float alpha)
	{
		vec3 aux;
	...
	Boids::vec3& vec3::multiply(const float& other)
	{
		x *= other;
		y *= other;
		z *= other;
	...
	Boids::vec3& vec3::operator*=(const float& other)
	{
	...
	std::ostream& operator<<(std::ostream& os, const Boids::vec3& vec)
	{
