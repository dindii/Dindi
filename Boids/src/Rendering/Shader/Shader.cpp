...
namespace Boids
{
	...
	void Shader::ParseShaderFiles(const std::string& vertexShaderPath, const std::string& fragmentShaderPath)
	{
		...
		{
			GLR_LOG_ERROR("ERROR READING SHADER FILES!");
		}
	...
	unsigned int Shader::CompileShader(unsigned int type, const std::string& source)
	{
		...
		if (!compileResult)
		{
			...
			if (type == GL_FRAGMENT_SHADER)
				GLR_LOG_ERROR("Failed to compile Fragment Shader: ", message);

			else if (type == GL_VERTEX_SHADER)
				GLR_LOG_ERROR("Failed to compile Vertex Shader: ", message);

			else if (type == GL_GEOMETRY_SHADER)
				GLR_LOG_ERROR("Failed to compile Geometry Shader: ", message);
