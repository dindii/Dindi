...
namespace GLR
{
	...
	GLR::vec3 vec3::lerp(const vec3& start, const vec3& end, float alpha)
	{
		vec3 aux;
	...
	GLR::vec3& vec3::multiply(const float& other)
	{
		x *= other;
		y *= other;
		z *= other;
	...
	GLR::vec3& vec3::operator*=(const float& other)
	{
	...
	std::ostream& operator<<(std::ostream& os, const GLR::vec3& vec)
	{
